...
export const mockPosts: PostDoc[] = [
  ...
  },
  {
    ...
    body: skimmableBlocks(
      ...
      ['## Why this matters for Australian buyers now'],
      [
        '• The current autocomplete pattern suggests Australian buyers are joining version-specific interest with pricing, inventory, and WMS questions. That usually means the shortlist is moving from awareness into practical validation.',
  ...
  },
  {
    ...
    _type: 'post',
    title:
      'NetSuite AI Connector Service in Australia: what finance and operations teams should test before connecting ChatGPT or Claude',
    slug: 'netsuite-ai-connector-service-australia-what-to-test',
    excerpt:
      'A practical Australian guide to NetSuite AI Connector Service, covering MCP scope, supported clients, permissions, tool limits, and the safest first pilot design.',
    category: 'NetSuite',
    publishedAt: '2026-06-10T00:00:00Z',
    updatedAt: '2026-06-10T00:00:00Z',
    seo: {
      metaTitle: 'NetSuite AI Connector Service Australia: What to Test First',
      metaDescription:
        'Australian buyer guide to NetSuite AI Connector Service using current Oracle documentation, AI Connector setup details, and AU search-interest signals.',
    },
    body: skimmableBlocks(
      [
        'Australian search intent around NetSuite AI has become more implementation-led in the last few weeks. On 10 June 2026, Google autocomplete with Australia targeting expanded queries such as "netsuite ai connector", "netsuite ai connector service", "netsuite ai connector service guide", "netsuite ai connector service cost", "netsuite ai connector claude", and "netsuite ai agents". That is a stronger buyer signal than a generic "does NetSuite have AI?" query because it suggests teams are now asking how external AI tools will actually connect to ERP records, reports, and searches.',
        'That makes NetSuite AI Connector Service a worthwhile site gap for ERP Search. The site already has a broader NetSuite AI article covering 2026.1 and roadmap caution, but it does not yet give Australian buyers a dedicated guide to the connector itself: what Oracle has officially released, which clients and permissions are involved, and what governance checks should happen before a finance or operations team opens live ERP access to an AI client.',
        'Oracle\'s current documentation is concrete enough to support a practical buying view. Oracle says NetSuite has adopted the Model Context Protocol, provides an MCP Standard Tools SuiteApp, supports connection paths for Claude and ChatGPT, and allows AI clients to create, read, and update records, run reports, run saved searches, and execute SuiteQL queries, with SuiteQL remaining read-only. That combination makes the connector commercially important. The question is no longer whether Oracle has an MCP story. The question is whether your team is ready to control one safely.',
      ],
      ['## What Oracle has officially released'],
      [
        '• Oracle\'s NetSuite AI Connector Service overview says NetSuite gives customers the option to use an external AI client, has adopted the Model Context Protocol, and provides the MCP Standard Tools SuiteApp plus the option to build custom tools.',
        '• Oracle\'s getting-started documentation says the connector is intended to let supported AI clients directly access and interact with NetSuite data and functionality.',
        '• Oracle\'s current FAQ says the standard tool set can create, read, and update records, run reports, run saved searches, and execute SuiteQL queries, with SuiteQL tools limited to read-only queries.',
        '• Oracle\'s NetSuite 2026.1 release notes, revision date 18 May 2026, include AI Connector Service-related updates such as support for consolidated reports and a new SuiteQL tool in MCP Standard Tools, plus enhancements to MCP Standard Tools and the AI Connector Service Companion SuiteApp.',
      ],
      ['## Why this matters for Australian teams now'],
      [
        '• This is one of the first Oracle-supported paths that makes bring-your-own-assistant ERP access concrete enough for shortlist, control, and budget discussions, not just innovation-theatre demos.',
        '• For lean Australian finance and operations teams, the attraction is obvious: an analyst can ask natural-language questions across live ERP records and saved searches without waiting for every reporting or workflow experiment to be custom-built first.',
        '• The risk is just as obvious. If an AI client can reach production records, reports, or searches, role design, approval boundaries, auditability, and data-handling expectations all become live operating questions rather than background IT plumbing.',
      ],
      ['## What the connector is good for'],
      [
        '• Giving users a governed way to retrieve operational and finance context through an AI client instead of depending on ad hoc exports or one-off integration prototypes.',
        '• Using standard NetSuite tools first. Oracle documents records, reports, saved searches, and SuiteQL as the default MCP Standard Tools scope, which is a useful starting point before a team decides it needs custom tools.',
        '• Supporting controlled multi-tool experiments. Oracle\'s documentation also points to custom tools and MCP Apps, which makes the connector relevant for teams thinking beyond a simple chat window and toward repeatable AI-assisted workflows.',
      ],
      ['## What it does not solve by itself'],
      [
        '• It does not make ERP governance easier automatically. The connector exposes tools, but it does not decide whether the user role, record access, or workflow boundaries are appropriate for AI-assisted use.',
        '• It does not remove the need for least-privilege design. Oracle explicitly recommends using a non-Administrator role and limiting permissions and tool access to only what is required.',
        '• It does not make every query path equivalent. Oracle says SuiteQL tools are read-only, which means teams still need to distinguish retrieval use cases from write-capable record scenarios.',
        '• It does not mean any AI client will work without checks. Oracle documents compatibility requirements around remote MCP, Protocol version 2025-06-18, streamable HTTP, and OAuth 2.0 Authorization Code Grant with PKCE.',
      ],
      ['## The setup and control points buyers should understand first'],
      [
        '• Client plan assumptions matter. Oracle\'s getting-started documentation says the Claude Pro plan or higher and the ChatGPT Plus plan or higher are needed to add the NetSuite AI Connector, while other clients need their own compatibility review.',
        '• Role design matters. Oracle\'s FAQ says to use a non-Administrator role and assign permissions including MCP Server Connection, OAuth 2.0 Access Tokens, and REST Web Services for the standard tools path.',
        '• Data sharing is explicit. Oracle\'s FAQ says users must acknowledge data sharing before connecting NetSuite to a third-party LLM, and the external model then handles data under that provider\'s privacy policy while NetSuite-side data remains under Oracle terms.',
        '• Monitoring still matters after go-live. Oracle\'s guidance says best practice includes using trusted AI clients, limiting permissions, restricting tool access, training users, and monitoring usage regularly.',
      ],
      ['## ChatGPT and Claude are supported, but not identical'],
      [
        '• Oracle publishes dedicated connection guidance for Claude and ChatGPT, which is a useful maturity signal because the setup is no longer hypothetical or partner-only.',
        '• Oracle\'s connect documentation says Claude users add the NetSuite AI connector inside claude.ai and then connect to the NetSuite account through the published server URL pattern.',
        '• Oracle\'s FAQ says Claude Pro and ChatGPT Plus or Pro are currently supported, but some client plans or versions need extra setup. Oracle specifically notes that some ChatGPT plans require Developer Mode to use MCP connectors.',
        '• That detail matters commercially. The project is not just "turn on AI". Someone still owns client-plan cost, support path, and which assistant becomes acceptable for production ERP use.',
      ],
      ['## What to test before approving any pilot'],
      [
        '• Start with one retrieval-led scenario. Good examples are summarising overdue invoices, surfacing open purchase orders for a supplier review, checking customer balances, or retrieving a controlled saved search for finance.',
        '• Prove permissions with real user roles, not an admin shortcut. If the pilot works only with broad access, the pilot has not proven a safe operating model.',
        '• Limit tools aggressively at first. Use the smallest set of records, reports, saved searches, or SuiteQL queries needed for the scenario, then review whether the outputs are still useful and supportable.',
        '• Test data-sharing and privacy expectations with the business owner, not only with IT. Oracle is explicit that third-party LLM handling follows the provider\'s privacy policy, so stakeholders should approve that model before rollout.',
        '• Separate read-heavy use cases from write-capable use cases. Oracle documents both retrieval and record updates, but the right first step for most Australian SMB and mid-market teams is a narrow read-focused pilot.',
        '• Decide how usage will be monitored before access expands. If nobody owns review of prompts, outputs, failed actions, and role appropriateness, the connector will outrun governance quickly.',
      ],
      ['## Where teams usually get this wrong'],
      [
        '• They treat the connector as a novelty demo instead of a production access design decision.',
        '• They assume the AI client subscription, NetSuite permissions, and data-sharing acknowledgments are minor setup details rather than commercial and governance decisions.',
        '• They jump too quickly from report retrieval to broad write-capable record access without proving business value in a safer read-led pilot first.',
        '• They ignore the difference between what Oracle supports at the protocol and connector layer and what their own business should actually allow users to do.',
      ],
      ['## The practical buyer conclusion'],
      [
        '• NetSuite AI Connector Service is now concrete enough to be a real 2026 evaluation topic for Australian finance and operations teams exploring external AI around ERP.',
        '• Oracle\'s documentation supports a sensible first move: one narrow pilot, one non-admin role, one bounded tool set, and one explicit data-sharing and monitoring design before broader rollout.',
        '• The wrong move is to treat the connector as a shortcut to autonomous ERP operations. The better move is to treat it as a governed access layer that can prove value safely before any broader write-capable workflow is considered.',
      ],
      ['## FAQ'],
      [
        '• Can NetSuite AI Connector Service work with ChatGPT or Claude? Yes. Oracle publishes connection guidance for Claude and ChatGPT, and its FAQ says Claude Pro and ChatGPT Plus or Pro are currently supported, though some versions need extra setup such as Developer Mode.',
        '• What can the standard connector do? Oracle\'s FAQ says it can create, read, and update records, run reports, run saved searches, and execute SuiteQL queries, with SuiteQL limited to read-only queries.',
        '• Should the first pilot allow record updates? Usually no. Oracle documents update capability, but its governance guidance and role recommendations support starting with a narrower, retrieval-led pilot first.',
        '• Does the connector remove privacy and control concerns? No. Oracle explicitly points users to associated risks, controls, and mitigation strategies and says third-party LLM data handling follows the provider\'s privacy policy.',
      ],
      ['## Sources used'],
      [
        '• Google autocomplete suggestion data with Australia targeting on 10 June 2026 for netsuite ai connector, netsuite ai connector service, netsuite ai connector service guide, netsuite ai connector service cost, netsuite ai connector claude, and netsuite ai agents.',
        '• Oracle NetSuite Help Center: Get Started with the NetSuite AI Connector Service.',
        '• Oracle NetSuite Help Center: NetSuite AI Connector Service overview.',
        '• Oracle NetSuite Help Center: Connect to the NetSuite AI Connector Service.',
        '• Oracle NetSuite Help Center: NetSuite AI Connector Service FAQ.',
        '• Oracle NetSuite 2026.1 Release Notes, revision date 18 May 2026, for MCP Standard Tools and Companion updates.',
      ],
    ),
  },
  {
