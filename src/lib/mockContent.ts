...
export const mockGuides: GuideDoc[] = [
  ...
  },
  {
    ...
    seo: {
      ...
      metaDescription:
        'Guide to AI agents for ERP covering order entry, bill processing, NetSuite, Business Central, SAP, Odoo, controls, permissions, and human review.',
    },
    body: skimmableBlocks(
      [
        'AI agents are becoming practical in ERP because they now sit close to the work that usually drains finance and operations teams: reading emails, interpreting attachments, drafting transactions, checking master data, and routing exceptions.',
        'The opportunity is real, but the control problem is just as real. ERP is where invoices become liabilities, orders become commitments, inventory promises become customer expectations, and master-data changes can affect every downstream process.',
        'A useful AI-agent roadmap therefore starts with automation and control together. The goal is not to remove people from ERP. The goal is to remove repetitive handling while keeping the right humans responsible for approvals, exceptions, policy, and audit evidence.',
      ],
      ['## What an ERP AI agent actually does'],
      [
        '• A basic assistant answers questions or drafts text. An ERP agent goes further: it can monitor an inbox, interpret a document, look up ERP records, prepare a transaction, ask for missing information, and route work for review.',
        '• Microsoft Business Central Sales Order Agent shows the pattern clearly. Microsoft says it analyses customer email requests, locates the customer, checks inventory, prepares quotes or orders, and keeps designated users involved for review and outgoing messages.',
        '• Microsoft Business Central Payables Agent shows the finance pattern. Microsoft says it monitors a mailbox, analyses vendor invoices, extracts content, and presents invoice drafts to supervisors for review before finalisation.',
        '• NetSuite is moving from embedded AI features toward connected AI clients through the NetSuite AI Connector Service. Oracle says it has adopted MCP so supported AI clients can interact with NetSuite data and functionality, with standard and custom tools available.',
        '• SAP describes Joule Agents as context-aware agents with business-process expertise that automate workflows across functions, while Odoo documents invoice digitisation using OCR and artificial intelligence to create and fill accounting records.',
      ],
      ['## The best first workflows are document-heavy and repetitive'],
      [
        '• Email-to-order: customer emails, purchase orders, attachments, item references, delivery dates, and follow-up questions can be turned into draft quotes or orders before a user reviews them.',
        '• Email-to-bill: supplier invoices, PDF attachments, vendor matching, coding suggestions, and draft purchase documents can be prepared before AP staff approve or post anything.',
        '• Shipping and receiving documents: bills of lading, packing slips, delivery dockets, and proof-of-delivery documents can be classified and matched to open ERP transactions.',
        '• Reconciliation support: bank lines, remittances, invoice references, and customer deductions can be suggested for matching while finance keeps final posting control.',
        '• Reporting and query agents: users can ask controlled questions over ERP data, but the data model, role permissions, and definition of approved metrics still matter.',
      ],
      ['## Where control must stay explicit'],
      [
        '• Human review: decide which records an agent may draft, which it may update, and which it must never post, release, approve, email, or pay without a person.',
        '• Role permissions: assign the agent only the access it needs. Microsoft documents agent permissions and profiles for Sales Order Agent, and Oracle points NetSuite AI Connector users toward required features, permissions, risks, and controls.',
        '• Source traceability: keep the original email, attachment, extracted values, matching logic, and reviewer decisions connected to the ERP record.',
        '• Exception queues: define what happens when the agent cannot identify a customer, item, vendor, tax code, price, order reference, or approval path confidently.',
        '• Segregation of duties: do not let one automation path create suppliers, change bank details, approve invoices, and release payments. The control model still needs separation.',
        '• Cost and capacity: some agent usage is consumption-based. If an agent monitors a busy mailbox, cost can follow transaction volume rather than user count.',
      ],
      ...
      [
        ...
        '• 5. Measure four things: successful drafts, exception rate, human correction effort, and control issues found.',
      ...
      [
        '• ERP process knowledge: the team must understand posting rules, order promises, inventory reservations, approval flows, tax, roles, and audit evidence, not only AI prompts.',
        '• Integration discipline: if an external agent or connector touches ERP data, the design needs monitoring, error handling, authentication, and support ownership.',
        '• Data-quality realism: agents can expose bad item descriptions, stale customer contacts, weak supplier records, and inconsistent coding rules. Those are business problems, not just model problems.',
        '• Release management: agent prompts, permissions, extensions, connectors, and ERP releases need regression testing like any other production capability.',
        '• Support ownership: someone must own failed agent tasks, mailbox failures, credit or usage limits, unexpected behaviour, and user training after go-live.',
      ],
      ...
      [
        ...
        '• ERP Search can help buyers shortlist partners and specialists who understand both sides of the work: ERP controls and practical automation delivery.',
      ],
      ['## FAQ'],
      [
      ...
      ['## Sources used'],
      [
        '• Microsoft Learn Sales Order Agent overview for Business Central email-to-quote and order processing, permissions, review points, and billing guidance.',
        '• Microsoft Learn Payables Agent overview for Business Central mailbox monitoring, invoice extraction, supervisor review, and draft purchase-document flow.',
        '• Oracle NetSuite AI Connector Service documentation for MCP-based AI client access, standard and custom tools, risks, controls, and permissions context.',
        '• SAP Joule Agents and Joule Assistants product page for agentic workflow, governance, and cross-functional automation positioning.',
        '• Odoo documentation for invoice digitisation using OCR and artificial intelligence to create and fill accounting records.',
      ],
    ),
  },
  {
