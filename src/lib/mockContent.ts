...
export const mockGuides: GuideDoc[] = [
  ...
  },
  {
    ...
    body: skimmableBlocks(
      ...
      [
        '• A tested operating model: products such as Dynamics 365, NetSuite, Epicor Kinetic, Odoo, SAP, Acumatica, and similar suites already carry established finance, purchasing, inventory, sales, manufacturing, reporting, and security patterns.',
        '• Vendor investment: SaaS ERP vendors keep adding product capability, AI features, compliance updates, integrations, reporting, mobile workflows, and security improvements.',
        '• Implementation ecosystem: buyers can access implementation partners, training material, documentation, marketplace apps, user communities, and known support paths.',
        '• Upgrade path: mature SaaS platforms are designed around vendor-managed releases. That does not make upgrades effortless, but it gives the business a maintained product core.',
        '• Configurable extension: NetSuite promotes SuiteCloud customisation for no-code changes, auditability, access management, and extensions to out-of-the-box capability. Epicor describes Kinetic Application Studio as no-code/low-code customisation for screens, fields, workflows, and upgrade-resilient user experience. Dynamics 365 guidance pushes fit-to-standard and fit-gap work before customisation.',
      ],
      ['## Where off-the-shelf ERP can fail the business'],
      [
        '• The business model is not normal for the product. A standard ERP may struggle where pricing, fulfilment, production, compliance, project billing, inventory ownership, service delivery, or customer contracts behave differently from the software assumptions.',
        '• The implementation forces poor process change. Some process standardisation is healthy. But forcing a business into a box can damage service, margin, speed, or compliance when the difference is part of how the company competes.',
        '• Workarounds multiply. If users need spreadsheets, side databases, manual re-keying, disconnected portals, and partner scripts to complete daily work, the SaaS core may be too narrow or the design may be wrong.',
        '• The extension model becomes fragile. Custom fields and workflows are not the same as a coherent product architecture. A heavily stretched ERP can become expensive to test, support, and explain.',
        '• The product roadmap does not match the business roadmap. If the vendor is moving toward generic cloud features while the business needs deep vertical capability, the gap may widen over time.',
      ],
      ['## What custom ERP can do better'],
      [
        '• Fit the operating model directly. A custom ERP can start with how the business sells, buys, fulfils, manufactures, services, invoices, reports, and controls risk, rather than starting from a vendor process map.',
        '• Remove unnecessary modules and screens. A specialist wholesaler, importer, manufacturer, project business, or field-service operator may only need a narrower system built around the work that actually matters.',
        '• Build the data model around the business. Custom ERP can encode unique product structures, service rules, margin logic, contract types, customer hierarchies, quality checkpoints, or fulfilment constraints.',
        '• Move faster on unusual workflows. AI-assisted builders and low-code platforms can produce prototypes quickly, making it easier to test a custom workflow before committing to a full system.',
        '• Own the product roadmap. If the business process is a competitive advantage, custom ERP can let the company prioritise its own features instead of waiting for vendor release cycles.',
      ],
      ['## What custom ERP makes harder'],
      [
        '• Finance and compliance depth: a full ERP is not just forms and dashboards. It needs posting rules, tax treatment, audit trails, approvals, reconciliation, security roles, period control, and reporting integrity.',
        '• Product ownership: someone must own design decisions, backlog priority, defects, documentation, testing, environments, releases, support, and training forever.',
        '• Integration reliability: banks, payroll, ecommerce, warehouses, 3PLs, EDI, tax, BI, and supplier/customer portals all become your integration estate rather than a vendor-supported ecosystem.',
        '• Upgrade responsibility: a custom system avoids vendor-mandated release cycles, but it does not avoid upgrades. Libraries, hosting, security patches, API changes, browser changes, AI models, and compliance rules still move.',
        '• Key-person risk: if one developer or agency understands the system, the business has built a new dependency rather than a durable platform.',
        '• Audit and investor confidence: lenders, auditors, acquirers, and enterprise customers may ask harder questions about a custom financial and operational system than a recognised ERP product.',
      ],
      ['## What AI-built ERP changes'],
      [
        '• AI can compress prototyping. Tools such as Softr now market AI-assisted custom ERP creation where users describe the app, generate a database and interface, connect data sources, manage roles, and iterate visually.',
        '• AI can help with workflow design, document handling, reporting, and user assistance. That can make custom internal tools more practical for smaller teams.',
        '• AI does not automatically create ERP-grade controls. The generated app still needs data governance, permission design, audit logs, workflow testing, exception handling, backups, disaster recovery, and ownership.',
        '• AI-built ERP is often strongest as a process app, portal, operational layer, or workflow system around the ERP core. Replacing the core finance and inventory system is a much bigger decision.',
      ],
      ['## Example scenarios'],
      [
      ...
      [
        ...
        '• Example: use Business Central, NetSuite, Epicor, Odoo, SAP, or Acumatica for finance, inventory ledger, purchasing, sales orders, tax, and reporting controls; build custom portals, quoting workflows, data capture, customer experiences, or AI-assisted exception handling around that core.',
        '• This mirrors the composable ERP direction Gartner describes: move away from static monolith thinking and design ERP capabilities that can flex with business change, automation, and AI without losing control of the core.',
