...
export const mockPosts: PostDoc[] = [
  ...
  {
    _id: 'post-bc-shopify-2026-au',
    _type: 'post',
    title: 'Business Central + Shopify in 2026: what Australian teams should test now',
    slug: 'business-central-shopify-2026-australia-what-to-test',
    excerpt: 'A practical guide to the latest Business Central Shopify connector changes for Australian teams managing product options, multi-currency orders, and catalogue control.',
    category: 'Business Central',
    publishedAt: '2026-05-08T00:00:00Z',
    updatedAt: '2026-05-08T00:00:00Z',
    seo: {
      metaTitle: 'Business Central + Shopify in 2026: What Australian Teams Should Test',
      metaDescription:
        'Australian guide to the latest Business Central Shopify connector updates, including checkout currency, custom collections, variant images, and catalogue testing priorities.',
    },
    body: skimmableBlocks(
      [
        'Australian ERP search intent is still clustering around practical product questions rather than generic ecommerce theory. One of the clearer overlaps right now is Business Central plus Shopify: product-led wholesalers, retailers, and hybrid B2B/B2C teams want tighter control of orders, pricing, inventory, and catalogue data without adding a separate integration project too early.',
        'Microsoft has quietly made the native Business Central Shopify connector more commercially relevant in the current release cycle. The important question is not whether the connector exists. It is whether the 2026 changes are strong enough for your operating model, especially if you sell across currencies, rely on product variants, or keep too much catalogue logic outside the ERP.',
        'This article focuses on the current Business Central documentation and 2026 release-wave items that matter most for Australian teams. The aim is to help buyers and existing customers decide what to test now, what to watch, and where manual work or process ambiguity will still break the model.',
      ],
      ['## What Microsoft officially supports now'],
      [
        '• Microsoft documents the Shopify Connector as a native link between Shopify stores and Business Central, supporting multiple shops, bidirectional synchronisation of products, inventory, orders, fulfilments, customers, prices, images, item variants, tags, and metafields.',
        '• Microsoft also documents background automation, logging, payout synchronisation, invoice export, and fulfilment tracking. That means the connector is no longer a narrow “import orders only” feature. It is a broader operating layer for ecommerce teams already inside the Microsoft stack.',
        '• For setup and testing, Microsoft explicitly recommends controlled test strategies because the connector can create or update items, customers, and sales documents. That warning matters more than the demo because it tells you where rollout risk actually sits.',
      ],
      ['## What changed in 2026 that deserves attention'],
      [
        '• Microsoft added support for using the customer checkout currency, or presentment currency, when Business Central creates sales documents from Shopify orders. This is positioned for Shopify Markets and other multi-currency selling scenarios.',
        '• Microsoft also added support for assigning custom Shopify collections from Business Central. That is useful when merchandising, category structure, or tax handling depends on more deliberate collection management instead of ad hoc manual updates in Shopify.',
        '• On the catalogue side, Microsoft added product options based on item attributes and support for synchronising variant-specific images between Business Central and Shopify. For teams with colour, size, material, or pack-configuration variants, that is much more useful than a cosmetic release-note line item.',
      ],
      ['## Why this matters for Australian teams'],
      [
        '• Multi-currency is not just an enterprise issue. Australian businesses increasingly sell across Australia, New Zealand, and other export markets through one commerce stack. If Shopify checkout currency and ERP sales-document currency diverge, finance and customer-service teams inherit the reconciliation pain.',
        '• Many growing wholesalers and retailers still maintain product attributes, images, or grouping logic partly in spreadsheets and partly in Shopify. Microsoft\'s newer connector features matter because they can reduce duplicate catalogue admin if the Business Central item model is already disciplined enough.',
        '• The native connector is also strategically relevant for buyers comparing Business Central with platforms that promise cleaner commerce integration out of the box. Release direction is useful shortlist evidence, even if the answer is still “test before trust”.',
      ],
      ['## What to test first in a serious evaluation'],
      [
        '• Currency handling. Test whether presentment currency improves order accuracy, credit memo handling, payment suggestion, and reconciliation for your real store setup. Do not treat it as a checkbox. Run it through refunds, returns, and close-period review.',
        '• Catalogue governance. Test whether item attributes in Business Central are clean enough to drive Shopify product options without creating malformed variants, duplicated option values, or support overhead.',
        '• Variant imagery. If merchandising depends on colourways, pack sizes, or finish variants, test whether image ownership belongs in Business Central and whether the sync removes enough manual work to justify the governance effort.',
        '• Collection logic. Review which collections should be governed centrally in Business Central and which should remain rule-driven in Shopify. Microsoft only documents manual assignment for custom collections, not smart collections, so teams need to separate those cases clearly.',
        '• Environment strategy. Use Microsoft\'s testing guidance properly. The connector can write data in both directions, so teams should decide early whether they are testing in Shopify sandbox, Business Central sandbox, or a controlled production-adjacent configuration with guardrails turned on.',
      ],
      ['## Where teams usually overestimate the native connector'],
      [
        '• They assume “native” means implementation-light. In practice, the hard work is still product data structure, location logic, fulfilment rules, customer handling, and exception recovery.',
        '• They expect the connector to repair weak product ownership. If attributes, images, pricing, or item relationships are unmanaged in Business Central, synchronisation just spreads the inconsistency faster.',
        '• They ignore multi-store complexity. Microsoft supports more than one shop, but each shop needs its own setup, products, locations, and pricing assumptions. That is an operating-model design question, not just a technical switch.',
        '• They confuse catalogue sync with warehouse readiness. A stronger Shopify connector does not remove the need to review bin control, dispatch flow, returns handling, or whether a dedicated WMS is still justified.',
      ],
      ['## A sensible first pilot'],
      [
        '• Start with one shop, one controlled product family, and one order flow that already has clean SKU, pricing, and fulfilment ownership.',
        '• Include at least one multi-currency order scenario if the business uses Shopify Markets or sells beyond Australia in a meaningful way.',
        '• Measure catalogue-maintenance effort, order exception rate, refund and return handling, inventory-sync confidence, and finance reconciliation effort before and after the pilot.',
        '• Review the pilot with ecommerce operations, customer service, finance, and whoever owns Business Central administration. This is a cross-functional operating-model decision, not an app-installation exercise.',
      ],
      ['## The practical conclusion'],
      [
        '• Business Central\'s Shopify connector deserves more serious attention in May 2026 than it did a year ago because Microsoft has added current, commercially relevant capabilities around currency handling, catalogue structure, and variant data.',
        '• The best fit is a business that already wants Business Central to act as the trusted core for item, order, and finance control rather than leaving those decisions fragmented across spreadsheets and storefront admins.',
        '• The biggest mistake is assuming the new features remove the need for disciplined product data, test environments, and exception ownership. They do not. They simply make a well-run connector strategy more viable.',
      ],
      ['## FAQ'],
      [
        '• Is the Business Central Shopify connector just for single-store setups? No. Microsoft documents support for more than one Shopify shop, but each shop has its own setup, products, locations, and pricing logic.',
        '• Does presentment currency matter for Australian businesses? Yes, if the business sells across currencies through Shopify Markets or other multi-currency storefront setups. It can materially affect reconciliation and customer-facing order accuracy.',
        '• Do the 2026 catalogue features mean Business Central now replaces all Shopify merchandising logic? No. They improve native sync for options, images, and collections, but teams still need clear ownership of product structure and smart-collection logic.',
        '• Should this topic matter to buyers who are not on Business Central yet? Yes. Microsoft\'s connector direction is useful evidence when comparing ERP options for product-led SMB operations that want tighter commerce integration.',
      ],
    ),
  },
  {
